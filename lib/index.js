...
module.exports = /** @class */ (function () {
		...
		return {
			...
			scripts: chunks.scripts
				.map(function (chunkJS) {
					return _this.options.templateScript.replace('{{chunk}}', chunkJS);
				})
				.join('')
		};
	};
	...
	ChunksWebpackPlugin.prototype.createHtmlChunksFiles = function (_a) {
		var entryName = _a.entryName;
		var htmlTags = _a.htmlTags;
		if (htmlTags.scripts.length) {
			utils.writeFile({
				outputPath:
					this.outputPath + '/' + entryName + '-scripts' + this.options.fileExtension,
		...
		if (htmlTags.styles.length) {
			utils.writeFile({
				outputPath:
					this.outputPath + '/' + entryName + '-styles' + this.options.fileExtension,
